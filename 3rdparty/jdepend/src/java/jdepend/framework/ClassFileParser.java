...
/**
 * The <code>ClassFileParser</code> class is responsible
...
public class ClassFileParser extends AbstractParser {
	...
	/**
	 * Constructs a <code>JavaClassFileParser</code> instance
	 ...
	 * @return
	 * @throws IOException
	 * @see jdepend.framework.AbstractParser#parse(java.io.InputStream)
	 */
	...
	public List<String> getDependencies() {
		List<String> result = new ArrayList<String>(constantPool.length);
		for (int i = 0; i < constantPool.length; i++) {
			if (constantPool[i] != null) {
				if (ClassFileParser.CONSTANT_CLASS == constantPool[i].getTag()) {
					result.add(constantPool[constantPool[i].getNameIndex()].getValue().toString());
				}
			}
		}
