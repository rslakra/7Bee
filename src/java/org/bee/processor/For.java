...
public class For extends AbstractBlock {
	...
	public InfoHolder eval() {
		...
		try {
			for (InfoHolder var : expandRange()) {
				getNameSpace().inScope(new InfoHolder<String, InfoHolder, Object>(variable, var));
			...
			switch (type) {
				...
				case directory:
					...
					if (fdir.exists() && fdir.isDirectory()) {
						...
						traverse(dirs, fdir);
						return arrayToInfoHolders(s, dirs.toArray(new String[dirs.size()]));
					}
					break;
				case variable:
				default:
	...
	public String[] getAllowedAttributeNames() {
		return Misc.merge(new String[] { ATTR_IN, ATTR_SEPARATOR }, super.getAllowedAttributeNames());
	}
}
